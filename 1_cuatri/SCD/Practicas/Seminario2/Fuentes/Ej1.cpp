...
const int nprod=1, ncons=1;
mutex
   mtx,
   mtx2 ;                 // mutex de escritura en pantalla
unsigned
...
void funcion_hebra_productora( ProdCons1SC * monitor, int nhebra )
{
   for( unsigned i = nhebra ; i < num_items ; i+=nprod )
   {
...
void funcion_hebra_consumidora( ProdCons1SC * monitor, int nhebra )
{
   for( unsigned i = nhebra ; i < num_items ; i+=ncons )
   {
